...
class ToolRegistry:
    ...
    def register(self, name: str, description: str, func: Any, schema: dict[str, Any]):
        ...
        self._schemas.append({"name": name, "description": description, "parameters": schema})
...
class ResearchAgent:
    ...
    async def run(self, bill_id: str, bill_text: str, jurisdiction: str) -> dict[str, Any]:
        ...
        available_tools_summary = [
            {"name": t["name"], "description": t["description"]} for t in self.registry._schemas
        ]

        logger.info(f"Generating research plan for {bill_id}...")
        plan = await self.planner.plan(
            query, context=context, available_tools=available_tools_summary
        )
