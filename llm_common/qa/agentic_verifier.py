    ...
    def verify(self, screenshot_path: str, story: str) -> VerificationResult:
        ...
        if not os.path.exists(screenshot_path):
            return VerificationResult(
                status="FAIL", reasoning=f"Screenshot not found at {screenshot_path}"
            )
        ...
        try:
            ...
            try:
                ...
                if "PASS" in clean_response and len(clean_response) < 50:
                    return VerificationResult(
                        status="PASS", reasoning="Model returned PASS without JSON."
                    )
                if "FAIL" in clean_response and len(clean_response) < 50:
                    return VerificationResult(
                        status="FAIL", reasoning=f"Model returned FAIL. Raw output: {result_json}"
                    )

                return VerificationResult(
                    status="FAIL",
                    reasoning=f"Model output parse error: {str(e)}. Raw output: {result_json}",
                )
