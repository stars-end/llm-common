...
class ServiceRegistry:
    ...
    def _detect_context(self) -> RuntimeContext:
        ...
        if os.environ.get("RAILWAY_ENVIRONMENT"):
            ...
            if "pr" in env or "pull" in env: return RuntimeContext.RAILWAY_PR
            if "production" in env: return RuntimeContext.RAILWAY_PROD
            return RuntimeContext.RAILWAY_DEV
        if os.environ.get("GITHUB_ACTIONS"): return RuntimeContext.GITHUB_CI
        return RuntimeContext.LOCAL

    def get_service_url(self, service_name: str, port: int = 8000) -> str:
        if service_name in self._overrides: return self._overrides[service_name]
        env_var = f"{service_name.upper()}_URL"
        if os.environ.get(env_var): return os.environ[env_var]
        if self._context == RuntimeContext.LOCAL: return f"http://localhost:{port}"
        if self._context == RuntimeContext.RAILWAY_PR: return f"http://{service_name}.railway.internal:{port}"
