...
async def test_research_agent_flow():
    ...
    mock_plan = ExecutionPlan(tasks=[
        PlannedTask(id=1, description="Research Test", sub_tasks=[SubTask(id=1, description="Search Google")])
    ])
    agent.planner.plan = AsyncMock(return_value=mock_plan)
    
    # Mock Executor Response
    mock_result = SubTaskResult(
        task_id=1, sub_task_id=0, success=True, 
