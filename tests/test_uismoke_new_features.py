...
from llm_common.agents.schemas import AgentStory, StoryResult
...
async def test_deterministic_only_skips_non_deterministic_steps():
    ...
    with patch.object(agent, "_verify_completion", new_callable=AsyncMock) as mock_verify:
        ...
        story = AgentStory(
            ...
            persona="tester",
            steps=[
                {"id": "s1", "navigate": "/home", "deterministic": True},  # Deterministic
                {"id": "s2", "description": "Complex action with LLM", "deterministic": False} # Non-deterministic
            ]
        )

        # Run with deterministic_only=True
        result = await agent.run_story(story, glm_client=MagicMock(), output_dir=Path("/tmp"), deterministic_only=True)
...
async def test_deterministic_only_fails_on_deterministic_failure():
    ...
    story = AgentStory(
        id="test-story",
        persona="tester",
        steps=[{"id": "s1", "navigate": "/home", "deterministic": True}]
    )
